...
import { addPromo, getDetailPromo, updatePromo } from "@/services/PromoService"
...
function EditPromoPage() {
  ...
  useEffect(() => {
    getDetailPromo(idPromo).then((data) => {
      setDataPromo(data)
      console.log(data)
    }).catch((error) => {
      Swal.fire({
        title: "Gagal!",
        text: "Kesalahan saat mengambil data promo!",
        icon: "error",
  ...
  useEffect(() => {
    setValue("nama", dataPromo?.nama)
    setValue("kode", dataPromo?.kode)
    setValue("persen", dataPromo?.persen)
    setValue("min_pembelian", dataPromo?.min_pembelian)
    setValue("max_potongan", dataPromo?.max_potongan)
    setDateRangeValue({
      startDate: dataPromo?.tanggal_mulai?.toDate(),
      endDate: dataPromo?.tanggal_akhir?.toDate()
    })
  }, [dataPromo])
  ...
  function submitForm(formData) {
    ...
    Swal.showLoading()
    updatePromo(idPromo, formData, dateRangeValue).then((res) => {
      if (res.status) {
        Swal.fire({
          title: "Sukses!",
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-center gap-2 p-5 md:px-0 ">
        ...
        <h3 className="text-xl font-semibold text-gray-800">Edit Promo</h3>
      </div>
